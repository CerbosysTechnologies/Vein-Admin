    ...
    <div class="relative overflow-hidden md:text-md text-lg">
      <div class="bg-white p-4 mt-2">
        <div class="flex justify-between items-center pr-2">
          ...
          <div>
            <a href="{{ route('admin.laboratory_location.create') }}">
              <button class="bg-primary text-white rounded-md  py-2 px-3 text-sm">+ New Lab location</button>
            </a>
          </div>
        </div>
            ...
            <table class="w-full rounded-md">
              <thead class="bg-light_blue rounded-md">
                <tr class="p-2 rounded-md font-semibold text-[0.9rem] whitespace-nowrap">
                  ...
                  <th class="px-4 font-semibold">Contact Person</th>
                  <th class="px-4 font-semibold">Contact</th>
                  <th class="px-4 font-semibold">Facility Type</th>
                  <th class="px-4 font-semibold">Image</th>

                  <th class="px-4 font-semibold">Action</th>
                ...
                @else
                    ...
                    @foreach ($laboratories as $laboratory)
                        <tr class="text-center text-[0.8rem]">
                            ...
                            <td class="text-sm">{{ $laboratory->name }}</td>
                            <td class="text-sm">{{ $laboratory->address }}</td>
                            <td class="text-sm">{{ $laboratory->contact_person_name }}</td>
                            <td class="text-sm">
                                @if ($laboratory->contacts)
                                    @foreach (json_decode($laboratory->contacts) as $contact)
                                        {{ $contact }}<br>
                                    @endforeach
                                @endif
                            </td>
                            <td class="text-sm">{{ $laboratory->facility_type }}</td>
                            <td> 
                              @if ($laboratory->image)
                                  <img src="{{ asset('lab_images/' . $laboratory->image) }}" alt="Laboratory Image" style="height: 80px; width: 80px; margin:4px;">
                              @else
                            ...
                            <td>
                              <div class="flex gap-2 justify-center">
                                  <a href="{{ route('admin.laboratory_location.edit', ['laboratory_location' => $laboratory->id]) }}">
                                      <i class="fa-solid fa-pen-to-square text-primary border border-primary text-sm p-0.2 px-1 rounded-[0.2rem]"></i>
                                  </a>
                                  <a>
                                    <i onclick="deleteOpen({{ $laboratory->id }}, 'laboratory')" class="fa-solid fa-trash-can text-red text-sm p-0.2 px-1 rounded-[0.2rem] border border-red"></i>
                                  </a>
                              </div>
                          </td>
                        </tr>
...
@endif
 ...
 <!------------------------- Delete POPUP HTRML -------------------------->
 <div id="delete-popup-laboratory" class="overflow-y-auto fixed top-0 hidden w-full z-50 flex justify-center items-center margin-auto h-full bg-black bg-opacity-60 pt-20 delete-popup">
    <!-- backdrop filter -->
    <div class="overflow-y-auto bg-white shadow-2xl opacity-100 rounded-2xl mx-8 sm:w-3/6 my-auto mt-20">
      <div class="flex justify-between items-center bg-light_gray  rounded-t-lg py-2.5">
        ...
        <i class="fa-regular fa-circle-xmark text-2xl px-2 pt-2" onclick="Delete_Close('laboratory')"></i>
      </div>
      ...
      <div class="flex items-center flex-col gap-5 p-5">
        ...
        <div class="font-bold flex justify-center flex-wrap gap-3">
          <div class="font-bold flex justify-center flex-wrap gap-3">
              <button class="bg-red text-white px-7 py-2 rounded text-sm font-light" onclick="Delete_Close('laboratory')"> No </button>
              @if($laboratories->isEmpty())
                  <p>No laboratorys found.</p>
              @else
                  <button class="bg-green text-white px-7 py-2 rounded text-sm font-light delete-button" data-item-id="{{ $laboratory->id }}" data-item-type="laboratory_location">Delete Test</button>
